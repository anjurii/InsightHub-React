...
describe('customerService', () => {
  it('filters customers by name, email, or company', async () => {
    await expect(getCustomers({ search: 'vertex' })).resolves.toEqual(expect.arrayContaining([expect.objectContaining({ name: 'Jackson Lee' })]))
  })
  ...
  it('creates a customer with a stable demo shape', async () => {
    ...
    expect(customer).toMatchObject({ name: 'Test Customer', email: 'test@customer.com', totalSpend: 0 })
  })
})

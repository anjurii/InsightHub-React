  ...
  return (
    <div className={dark ? 'app dark' : 'app'}>
      <aside className={`sidebar ${collapsed ? 'collapsed' : ''} ${mobileOpen ? 'mobile-open' : ''}`}>
        ...
        <nav>{navItems.map(({ label, icon: Icon }) => <button className={view === label ? 'nav-item active' : 'nav-item'} key={label} onClick={() => { navigate(label === 'Overview' ? '/' : `/${label.toLowerCase()}`); setMobileOpen(false) }}><Icon size={18} /><span>{label}</span>{label === 'Customers' && <em>5</em>}</button>)}</nav>
        <p className="nav-label">Manage</p>
        <nav><button className="nav-item" onClick={() => downloadCsv('insighthub-report.csv', [['Metric', 'Value'], ['Revenue', '$842,930'], ['Customers', '24,892'], ['Orders', '8,492']])}><FileText size={18} /><span>Reports</span></button><button className="nav-item" onClick={() => notify('Settings are available from the theme control above')}><Settings size={18} /><span>Settings</span></button></nav>
        <div className="sidebar-bottom"><button className="help-card" onClick={() => notify('Help center: try the search and filters to explore your data')}><CircleHelp size={18} /><div><strong>Need a hand?</strong><small>Check our help center</small></div><ChevronRight size={14} /></button><button className="profile" onClick={logout}><div className="profile-avatar">AK</div><div className="profile-copy"><strong>Alex Kim</strong><small>Admin · Sign out</small></div><MoreHorizontal size={17} /></button></div>
      </aside>
...
function Overview({ notify, downloadCsv }: { notify: (message: string) => void; downloadCsv: (filename: string, rows: string[][]) => void }) {
  ...
  const chartData = range === 'Last 6 months' ? revenueData.slice(-6) : range === 'This year' ? revenueData.slice(0, 12) : revenueData
  return <><PageHeading title="Overview" description="A clear view of how your business is performing." action={<div className="heading-actions"><label className="button secondary"><CalendarDays size={16} /><select className="inline-select" value={range} onChange={e => setRange(e.target.value)}><option>Last 12 months</option><option>Last 6 months</option><option>This year</option></select><ChevronDown size={14} /></label><button className="button primary" onClick={() => downloadCsv('insighthub-overview.csv', [['Metric', 'Value'], ['Total revenue', '$842,930'], ['Total customers', '24,892'], ['Total orders', '8,492'], ['Conversion rate', '3.48%']])}><Download size={16} /> Export report</button></div>} />
...
function Customers({ notify }: { notify: (message: string) => void }) {
  const navigate = useNavigate()
  const [search, setSearch] = useState('')
  const [statusFilter, setStatusFilter] = useState('All')
  const [sortAsc, setSortAsc] = useState(true)
  const [selected, setSelected] = useState<Customer | null>(null)
  const { data: fetched = [], isLoading, isError } = useCustomers(search)
  const visible = useMemo(() => fetched.filter((customer) => statusFilter === 'All' || customer.status === statusFilter).sort((a, b) => sortAsc ? a.name.localeCompare(b.name) : b.name.localeCompare(a.name)), [fetched, sortAsc, statusFilter])
  return <><PageHeading title="Customers" description="Manage relationships and understand your audience." action={<button className="button primary" onClick={() => navigate('/customers/new')}><Plus size={17} /> Add customer</button>} /><section className="card table-card"><div className="table-toolbar"><div className="search"><Search size={17} /><input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search customers..." /></div><div className="toolbar-actions"><label className="button secondary"><SlidersHorizontal size={16} /><select className="inline-select" value={statusFilter} onChange={e => setStatusFilter(e.target.value)}><option>All</option><option>Active</option><option>Pending</option><option>Inactive</option></select><ChevronDown size={14} /></label><button className="button secondary" onClick={() => notify('Customer data export is ready')}><Download size={16} /> Export</button></div></div>{isLoading ? <div className="table-state"><div className="skeleton-line" /><div className="skeleton-line" /><div className="skeleton-line" /></div> : isError ? <div className="table-state"><strong>Unable to load customers</strong><span>Please try again in a moment.</span></div> : visible.length === 0 ? <div className="table-state"><strong>No customers found</strong><span>Try a different search or status.</span></div> : <div className="table-scroll"><table><thead><tr><th><button className="table-sort" onClick={() => setSortAsc(!sortAsc)}>Customer <ChevronDown size={13} /></button></th><th>Company</th><th>Total spend</th><th>Status</th><th>Joined</th><th /></tr></thead><tbody>{visible.map(c => <tr key={c.email}><td><button className="person person-button" onClick={() => setSelected(c)}><div className={`avatar ${c.color}`}>{c.initials}</div><div><strong>{c.name}</strong><small>{c.email}</small></div></button></td><td>{c.company}</td><td><strong>{c.spend}</strong></td><td><span className={`status ${c.status.toLowerCase()}`}><i />{c.status}</span></td><td>{c.date}</td><td><button className="more-btn" onClick={() => setSelected(c)}><MoreHorizontal size={18} /></button></td></tr>)}</tbody></table></div>}<div className="pagination"><span>Showing <strong>1–{visible.length}</strong> of <strong>2,481</strong> customers</span><div><button className="page-btn"><ChevronLeft size={16} /></button><button className="page-btn active">1</button><button className="page-btn">2</button><button className="page-btn">3</button><span>...</span><button className="page-btn">248</button><button className="page-btn"><ChevronRight size={16} /></button></div></div></section>{selected && <div className="modal-backdrop" onClick={() => setSelected(null)}><section className="customer-modal" onClick={event => event.stopPropagation()}><button className="modal-close" onClick={() => setSelected(null)}><X size={18} /></button><div className={`avatar ${selected.color} modal-avatar`}>{selected.initials}</div><h2>{selected.name}</h2><p>{selected.email}</p><div className="detail-grid"><span>Company<strong>{selected.company}</strong></span><span>Status<strong>{selected.status}</strong></span><span>Total spend<strong>{selected.spend}</strong></span><span>Joined<strong>{selected.date}</strong></span></div><button className="button primary modal-action" onClick={() => { notify('Edit form coming next'); setSelected(null) }}>Edit customer</button></section></div>}</>
}

function Orders({ notify }: { notify: (message: string) => void }) {
  const [search, setSearch] = useState('')
  const [status, setStatus] = useState('All')
  const visibleOrders = orders.filter((order) => `${order.id} ${order.customer} ${order.product}`.toLowerCase().includes(search.toLowerCase()) && (status === 'All' || order.status === status))
  return <><PageHeading title="Orders" description="Keep track of every transaction in one place." action={<button className="button primary" onClick={() => notify('Order form coming next')}><Plus size={17} /> New order</button>} /><div className="mini-stats"><button className="mini-stat mini-stat-button" onClick={() => setStatus('All')}><span className="mini-icon purple"><Package size={17} /></span><div><small>All orders</small><strong>8,492</strong></div><span className="mini-change">+8.4%</span></button><button className="mini-stat mini-stat-button" onClick={() => setStatus('Processing')}><span className="mini-icon orange"><TrendingUp size={17} /></span><div><small>Processing</small><strong>284</strong></div><span className="mini-change">+4.2%</span></button><button className="mini-stat mini-stat-button" onClick={() => setStatus('Completed')}><span className="mini-icon mint"><CreditCard size={17} /></span><div><small>Completed</small><strong>7,912</strong></div><span className="mini-change">+10.1%</span></button><button className="mini-stat mini-stat-button" onClick={() => setStatus('Refunded')}><span className="mini-icon rose"><ArrowDownRight size={17} /></span><div><small>Refunded</small><strong>296</strong></div><span className="mini-change negative">-2.8%</span></button></div><section className="card table-card"><div className="table-toolbar"><div className="search"><Search size={17} /><input value={search} onChange={event => setSearch(event.target.value)} placeholder="Search orders..." /></div><div className="toolbar-actions"><label className="button secondary"><SlidersHorizontal size={16} /><select className="inline-select" value={status} onChange={event => setStatus(event.target.value)}><option>All</option><option>Processing</option><option>Completed</option><option>Refunded</option></select><ChevronDown size={14} /></label><button className="button secondary" onClick={() => notify('Date range filter ready for API data')}><CalendarDays size={16} /> Date range</button></div></div>{visibleOrders.length === 0 ? <div className="table-state"><strong>No orders found</strong><span>Try another search or status.</span></div> : <div className="table-scroll"><table><thead><tr><th>Order ID</th><th>Customer</th><th>Product</th><th>Amount</th><th>Status</th><th>Date</th><th /></tr></thead><tbody>{visibleOrders.map(o => <tr key={o.id}><td><strong className="order-id">{o.id}</strong></td><td>{o.customer}</td><td>{o.product}</td><td><strong>{o.amount}</strong></td><td><span className={`status ${o.status.toLowerCase()}`}><i />{o.status}</span></td><td>{o.date}</td><td><button className="more-btn" onClick={() => notify(`${o.id} selected`)}><MoreHorizontal size={18} /></button></td></tr>)}</tbody></table></div>}<div className="pagination"><span>Showing <strong>1–{visibleOrders.length}</strong> of <strong>8,492</strong> orders</span><div><button className="page-btn"><ChevronLeft size={16} /></button><button className="page-btn active">1</button><button className="page-btn">2</button><button className="page-btn">3</button><span>...</span><button className="page-btn">850</button><button className="page-btn"><ChevronRight size={16} /></button></div></div></section></>
}
